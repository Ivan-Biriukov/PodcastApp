...
class ChannelTableViewCell: UITableViewCell {
    ...
    private func addSubviews() {
        ...
        [avatarImageView, mainTitleStack].forEach({bubbleView.addSubview($0)})
        [podcastNameLabel, botLabelsStack].forEach({mainTitleStack.addArrangedSubview($0)})
        [totalDurationLabel, doteViewSeporator, episodsCountLabel].forEach({botLabelsStack.addArrangedSubview($0)})
    }
    ...
    func fill(viewModel: Episode) {
        urlString = viewModel.episodeImgURLString
        avatarImageView.kf.indicatorType = .activity
        avatarImageView.kf.setImage(with: URL(string: urlString))
        podcastNameLabel.text = viewModel.name
        totalDurationLabel.text = "\(viewModel.timeDuration / 60)" + ":" + "\(viewModel.timeDuration % 60)"
        episodsCountLabel.text = viewModel.episodesCount + " " + "Eps"
    }
}
